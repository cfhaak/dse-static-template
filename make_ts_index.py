...
try:
    client.collections["short_project_title"].delete()
except ObjectNotFound:
    pass

current_schema = {
    "name": "short_project_title",
    "fields": [
        {"name": "id", "type": "string"},
...
for x in tqdm(files, total=len(files)):
    ...
    for v in facs:
        ...
        pages += 1
        cfts_record = {
            "project": "short_project_title",
        }
        record = {}
        record["id"] = os.path.split(x)[-1].replace(".xml", f".html?tab={str(pages)}")
        cfts_record["id"] = record["id"]
        cfts_record["resolver"] = f"github_code_repo_url/{record['id']}"
...
print(make_index)
print("done with indexing short_project_title")

make_index = CFTS_COLLECTION.documents.import_(cfts_records, {"action": "upsert"})
print(make_index)
print("done with cfts-index short_project_title")
